...
def main():
    ...
    lr = "lr1e-5"
    evals = {
        extract_steps(file): PredictionEvaluation.load(f"{Paths.PREDICTIONS}epoch_eval_{lr}/{file}")
        for file in os.listdir(Paths.PREDICTIONS + "/epoch_eval_" + lr)
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"{Paths.PLOTS}epoch_eval_{lr}.png")
