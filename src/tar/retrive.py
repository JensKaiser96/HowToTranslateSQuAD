...
def retrieve(source_text: str, source_span: Span, target_text: str) -> Span:
    """
    ...
    alignment, source_tokens, target_tokens = align(source_text, target_text)

    # get surface token mapping for both source and target
    source_surface_token_mapping = surface_token_mapping(source_text, source_tokens)
    target_surface_token_mapping = surface_token_mapping(target_text, target_tokens)

    source_span_token_ids = [
        index
        for index, span in enumerate(source_surface_token_mapping)
    ...
    target_surface_spans = [
        span
        for index, span in enumerate(target_surface_token_mapping)
